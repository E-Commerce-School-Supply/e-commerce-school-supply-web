...
const userAvatarSrc = computed(() => {
	...
	if (!candidate) return BlankProfile
	const s = String(candidate).trim()
	if (!s) return BlankProfile

	// Absolute URLs / data / blob should be used as-is
	if (/^(https?:\/\/|data:image\/|blob:)/.test(s)) return s

	// App-root absolute path: prefix API base
	if (s.startsWith('/')) {
		const base = API_BASE_URL || ''
		return `${String(base).replace(/\/$/, '')}${s}`
	}

	// Relative paths (./ or ../) - prefix API base
	if (/^\.{1,2}\//.test(s)) {
		const base = API_BASE_URL || ''
		return `${String(base).replace(/\/$/, '')}/${s.replace(/^\.{1,2}\//, '')}`
	}
