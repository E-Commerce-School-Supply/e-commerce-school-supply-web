...
<template>
  <div class="border border-default border-dashed rounded-base p-5 mt-5">
    <h1 v-if="form.setName" class="text-accent">Edit your address</h1>
    <h1 v-else class="text-accent">Add a new address</h1>
    ...
    <form class="w-full max-w-4xl mx-auto mt-5" @submit.prevent="onSave">
      <div class="relative z-0 w-full mb-6 group">
        ...
        <label for="name" class="absolute text-sm text-body duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-left">{{ $t('profile.address_name') }}</label>
      </div>
      ...
      <div class="grid md:grid-cols-2 md:gap-6">
        <div class="relative z-0 w-full mb-6 group">
          ...
          <label for="country" class="absolute text-sm text-body duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-left">{{ $t('checkout.country') }}</label>
        </div>
        <div class="relative z-0 w-full mb-6 group">
          ...
          <label for="province" class="absolute text-sm text-body duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-left">{{ $t('checkout.province_city') }}</label>
        </div>
      </div>
      ...
      <div class="relative z-0 w-full mb-6 group">
        ...
        <label for="address1" class="absolute text-sm text-body duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-left">{{ $t('checkout.address_line_1') }}</label>
      </div>
      ...
      <div class="grid md:grid-cols-2 md:gap-6">
        <div class="relative z-0 w-full mb-6 group">
          ...
          <label for="house" class="absolute text-sm text-body duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-left">{{ $t('checkout.house_number') }}</label>
        </div>
        <div class="relative z-0 w-full mb-6 group">
          ...
          <label for="street" class="absolute text-sm text-body duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-left">{{ $t('checkout.street') }}</label>
        </div>
      </div>
      ...
      <div class="relative z-0 w-full mb-6 group">
        ...
        <label for="address2" class="absolute text-sm text-body duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-left">{{ $t('checkout.address_line_2') }}</label>
      </div>
      ...
      <div class="relative z-0 w-full mb-6 group">
        ...
        <label for="zip" class="absolute text-sm text-body duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-left">{{ $t('checkout.zip_code') }}</label>
      </div>

      <div class="mt-3 w-full flex flex-row gap-2 items-center justify-end">
        <button type="button" @click="onCancel" class="text-body bg-neutral-primary border border-default hover:bg-neutral-secondary-soft hover:text-heading focus:ring-4 focus:ring-neutral-tertiary font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">{{ $t('profile.cancel') }}</button>
        <button type="submit" class="text-white bg-secondary box-border border border-transparent hover:bg-secondary/80 focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">{{ $t('profile.save') }}</button>
      </div>
    </form>
  </div>
</template>
